export async function onRequestPost(context) {
  ...
  try {
    ...
    const lastMonetagClaim = fields.lastMonetagClaim ? parseInt(fields.lastMonetagClaim.integerValue || '0', 10) : 0;
    
    if (now - lastMonetagClaim < TASK_COOLDOWN_MS) {
      return new Response(JSON.stringify({ 
        success: false, 
        error: 'Cooldown active',
        remainingMs: TASK_COOLDOWN_MS - (now - lastMonetagClaim)
    ...
    const updatedUserFields = {
      fields: {
        ...
        lastMonetagClaim: { integerValue: now.toString() }
      }
    };
